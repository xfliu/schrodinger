...
function stage_mu2(c::Config, geo, nodes, wts, prefix, Ceps, L1, L2)
    ...
    L2_LG = (rho_p - rho_p/(interval(1.0)-nu)) - interval(CSHIFT())
...
function stage_lg(c::Config, geo, nodes, wts, prefix, L1, L2, mu2_cert)
    ...
    function mobius(rr::Float64)
        rp = interval(rr) + interval(CSHIFT())
        Ax = A0 - rp
        Bx = A0 - interval(2.0)*rp + rp*rp*A2
        nux = Ax/Bx
...
function main()
    ...
    recA, mu1, mu2, Ceps, L1, L2 = stage_A(c, geo, nodes, wts, prefix)
    ...
    GC.gc(true); recD, lam_hi   = stage_dir(c, geo, nodes, wts, prefix)
