...
function run_N(N::Int; nt::Int=48, npanel::Int=48, eps_grid=[0.40,0.30],
               nodes::Vector{IV}, wts::Vector{IV})
    T0 = time()
    # ---------- EEE block ----------
    t_as1 = @elapsed (Peee,Keee,Deee) = AV.assemble_PK(N,:even,:even,:even; nt=nt,npanel=npanel,nodes=nodes,wts=wts)
    ...
    # ---------- OEE block ----------
    t_as2 = @elapsed (Poee,Koee,Doee) = AV.assemble_PK(N,:odd,:even,:even; nt=nt,npanel=npanel,nodes=nodes,wts=wts)
